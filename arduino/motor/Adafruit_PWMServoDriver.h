...
//  uint16_t getPulse(void);
    uint16_t getPulse(int channel);
  void on_check(int no);
  void convert_check(int no);
  bool on[6];
  long int gap;
  long int st; //start time; when constructor is made
  int servomax;
  int servomin;
  bool forward[6];
  void rotate(int no);
...
private:
  ...
  long int ct; //current time to know if it's ready to run
};
