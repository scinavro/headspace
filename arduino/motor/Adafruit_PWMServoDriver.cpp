...
//{
//    _i2c->beginTransmission(_i2caddr);
//    current_pulse = _i2c->read();
//  return current_pulse;
//}
uint16_t Adafruit_PWMServoDriver::getPulse(int channel)
{
    byte regAddress = PCA9685_LED0_ON_L + ((uint8_t)channel << 2);
    _i2c->beginTransmission(_i2caddr);
...
void Adafruit_PWMServoDriver::convert_check(int no)
{
  if (forward[no])
  {
...
void Adafruit_PWMServoDriver::rotate(int no) //no: gap*no
{
  convert_check(no);
  if (on[no])
  {
