...
Adafruit_PWMServoDriver::Adafruit_PWMServoDriver()
    : _i2caddr(PCA9685_I2C_ADDRESS), _i2c(&Wire), st(millis()), gap(1500), forward(true), on(false), servomax(600), servomin(150), step(1) {}
...
uint16_t Adafruit_PWMServoDriver::getPulse(void)
{
  return current_pulse;
}
void Adafruit_PWMServoDriver::on_check(int no)
{
  ct = millis();
  if (ct - st >= gap*no)
    on = true;
...
void Adafruit_PWMServoDriver::convert_check()
{
  if (forward)
  {
    if (getPulse() > servomax)
      forward = false;
  }
  else
  {
    if (getPulse() < servomin)
      forward = true;
  }
}

void Adafruit_PWMServoDriver::rotate(uint8_t num, int no) //no: gap*no
{
  convert_check();
  if (on)
  {
